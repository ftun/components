/**
* ======================================================= Compare Strings =======================================================
* Funcion obtener las diferencias entre dos textos
* @author Angel Solis <angesolis@palaceresorts.com>
*
* @version 1.0
*
* @param before: string (required) Texto Original.
* @param after: string (required) Texto Nuevo.
* @param validation: integer (optional) Tipo de Validacion. Default es NORMAL = 0.
*        {
* @param    NORMAL  0 : Se valida el texto que se inserto y el texto que se elimino.
* @param    INSERTS 1 : Se valida solo el texto que se agrego.
* @param    DELETES 2 : Se valida solo el texto que se elimino.
*        }
*
* @method CompareStrings(before,after,validation): Función que retorna un string.
*
* @example use:
*       import CompareStrings from '@ftun/comparestrings';
*       let text= CompareStrings("<h1>Hello</h1>","<H1>Hello World</H1>");
*
* @return string. texto que se valido.

/**
* @property Object. Tipo de Validación
*/
const VALIDATIONCOMPARESTRINGS = {
	NORMAL : 0,
	INSERTS : 1,
	DELETES : 2
};
...
/**
 * @property Integer. Tipo de Validación que se validará
 */
let VALIDATIONSELECTEDCOMPARESTRINGS= VALIDATIONCOMPARESTRINGS.NORMAL;

/**
 * @property Object. estilos para las etiquetas.
 */
const STYLESCOMPARESTRINGS = {
	ins : 'text-decoration:none;background-color:#d4fcbc',
	del : 'text-decoration:line-through;background-color:#fbb6c2;color:#555'
};

let CompareStrings = (before, after, validation=VALIDATIONCOMPARESTRINGS.NORMAL)=> {
	let ops;
	VALIDATIONSELECTEDCOMPARESTRINGS=validation;
	if (before === after) {
		return before;
	}
...
let is_end_of_tag = (char)=> { return char === '>'; };

let is_start_of_tag = (char)=> { return char === '<'; };

let is_whitespace = (char)=> { return /^\s+$/.test(char); };

let is_tag = (token)=> { return /^\s*<[^>]+>\s*$/.test(token); };

let isnt_tag = (token)=> { return !is_tag(token); };

let MatchCompare = (()=> {
	function MatchCompare(start_in_before1, start_in_after1, length1) {
...
let find_match =(before_tokens, after_tokens, index_of_before_locations_in_after_tokens, start_in_before, end_in_before, start_in_after, end_in_after) => {
	let best_match_in_after, best_match_in_before, best_match_length, i, index_in_after, index_in_before, j,
	len, locations_in_after, looking_for, match, match_length_at, new_match_length, new_match_length_at, ref, ref1;
	...
	if (best_match_length !== 0) {
		match = new MatchCompare(best_match_in_before, best_match_in_after, best_match_length);
	}
	return match;
};

let recursively_find_matching_blocks = (before_tokens, after_tokens, index_of_before_locations_in_after_tokens, start_in_before, end_in_before, start_in_after, end_in_after, matching_blocks)=> {
	let match;
...
let find_matching_blocks = (before_tokens, after_tokens)=> {
...
let calculate_operations = (before_tokens, after_tokens)=> {
	let action_map, action_up_to_match_positions, i, index, is_single_whitespace, j, last_op, len, len1, match,
	...
	matches.push(new MatchCompare(before_tokens.length, after_tokens.length, 0));
...
let consecutive_where = (start, content, predicate)=> {
...
let wrap = (tag, content)=> {
	...
	while (true) {
		...
		if (non_tags.length !== 0) {
			rendering +=`<${tag} style='${STYLESCOMPARESTRINGS[tag]}'>` + (non_tags.join('')) +`</${tag}>`;
		}
		if (position >= length) {
			break;
		}
...
let op_map = {
	...
	insert: function(op, before_tokens, after_tokens) {
		let val = after_tokens.slice(op.start_in_after, +op.end_in_after + 1 || 9e9);
		return (VALIDATIONSELECTEDCOMPARESTRINGS == VALIDATIONCOMPARESTRINGS.NORMAL || VALIDATIONSELECTEDCOMPARESTRINGS == VALIDATIONCOMPARESTRINGS.INSERTS) ? wrap('ins', val) : "";
	},
	"delete": function(op, before_tokens, after_tokens) {
		let val = before_tokens.slice(op.start_in_before, +op.end_in_before + 1 || 9e9);
		return (VALIDATIONSELECTEDCOMPARESTRINGS == VALIDATIONCOMPARESTRINGS.NORMAL || VALIDATIONSELECTEDCOMPARESTRINGS == VALIDATIONCOMPARESTRINGS.DELETES) ? wrap('del', val) : "";
	},
	replace: function(op, before_tokens, after_tokens) {
		return (op_map["delete"](op, before_tokens, after_tokens)) + (op_map.insert(op, before_tokens, after_tokens));
	}
};

let render_operations = (before_tokens, after_tokens, operations)=> {
...
CompareStrings.html_to_tokens = html_to_tokens;

CompareStrings.find_matching_blocks = find_matching_blocks;

find_matching_blocks.find_match = find_match;

find_matching_blocks.create_index = create_index;

CompareStrings.calculate_operations = calculate_operations;

CompareStrings.render_operations = render_operations;
